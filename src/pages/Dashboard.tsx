...
const Dashboard: React.FC = () => {
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useQuery('analytics', analyticsApi.getDashboardStats);
  
  // Process analytics data for charts
  const userGrowthData = analytics?.data?.data?.userGrowth?.map((item: any) => ({
  ...
  const reportData = analytics?.data?.data?.reportTypes?.map((item: any) => ({
  ...
  if (analyticsError) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-400">Error details: {analyticsError instanceof Error ? analyticsError.message : 'Unknown error'}</p>
      </div>
    );
  }
          ...
          {userGrowthData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No user growth data</h3>
                <p className="text-gray-500">User registration data will appear here.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">Distribution of report categories</p>
            </div>
            <div className="flex items-center space-x-2">
              <ChartBarIcon className="h-5 w-5 text-purple-500" />
              <span className="text-sm font-medium text-purple-600">{totalReports} total</span>
            </div>
          </div>
          {reportData.length > 0 ? (
                ...
                {reportData.map((item: any, index: number) => (
                  <div key={index} className="flex items-center">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">{item.value} reports</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-500">Report data will appear here when available.</p>
              </div>
            </div>
          )}
        </div>
